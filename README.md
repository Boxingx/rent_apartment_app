...
При регистрации как в случае арендодателя так и в случае арендатора, пользователям им присваивается временной токен, сделано что то на подобии сессии, пользователь должен будет передавать этот токен в хедерах, что бы использовать функционал приложения. Этот токен проходит проверку по дате, не истек ли он.
